...
private struct FlagRegister
{
    ...
    mixin(bitfields!(
        uint, "_", 4,
        bool, "carryFlagC", 1,
        bool, "halfCarryFlagH", 1,
        bool, "subtractFlagN", 1,
        bool, "zeroFlagZ", 1
    ));
...
unittest
{
    ...
    union U { ubyte value; FlagRegister flags; }
    U u = { value: 0x00 };
    ...
    assert(u.value == 0b0010_0000, "The half carry flag bit is the all but two most significant bit");
...
unittest
{
    ...
    assert(registers.programCounter == 0x0100, "The program counter should be initialised to 0x0100.");
}